...
def broadcast_exp(x, n_formulas, input_dim):
    mu = tf.compat.v1.get_variable('exp_mu', [n_formulas, input_dim], initializer=tf.initializers.random_normal())
    sigma = tf.compat.v1.get_variable('exp_sigma', [1, n_formulas, input_dim], initializer=tf.initializers.random_normal())
...
def feature_selection(input_dim, keep_feature_prob_arr, n_literals_per_formula_arr, n_formulas, elastic_net_beta):
    ...
    elastic_net_alpha = tf.compat.v1.get_variable('elastic_net_alpha', initializer=tf.constant(value=0.))
    learnable_mask = tf.compat.v1.get_variable('learnable_mask', [input_dim, n_formulas], initializer=tf.initializers.constant(binary_threshold_eps + 0.5))

    learnable_mask_01 = binary_threshold(learnable_mask, eps=binary_threshold_eps)

    l2_square_norm_selected = tf.linalg.tensor_diag_part(tf.matmul(tf.transpose(tf.square(learnable_mask)), formulas_random_mask))
    l1_norm_selected = tf.linalg.tensor_diag_part(tf.matmul(tf.transpose(tf.abs(learnable_mask)), formulas_random_mask))
