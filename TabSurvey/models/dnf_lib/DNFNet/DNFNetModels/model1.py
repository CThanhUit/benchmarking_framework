...
from models.dnf_lib.DNFNet.DNFNetModels.DNFNetComponents import compute_total_number_of_literals, compute_total_number_of_conjunctions, compute_n_literals_per_formula,\
    denseNDArrayToSparseTensor, create_conjunctions_indicator_matrix, create_formulas_indicator_matrix, feature_selection, and_operator, or_operator, broadcast_exp, tf_matmul_sparse_dense
...
###########################################################################################
# complete DNF-Net: DNF structure with feature selection and localization
###########################################################################################
class MNN(object):
    ...
    def build(self, placeholders, n_labels, phase):
        ...
        bias = tf.compat.v1.get_variable('literals_bias', [total_number_of_literals], initializer=tf.zeros_initializer())
        weight = tf.compat.v1.get_variable('literals_weights', [input_dim, total_number_of_literals], initializer=tf.contrib.layers.xavier_initializer())
        ...
        temperature = tf.compat.v1.get_variable('temperature', initializer=tf.constant(value=2.))
        ...
        with tf.compat.v1.variable_scope('loss'):
            if n_labels > 1:
                ...
                loss = tf.compat.v1.losses.sigmoid_cross_entropy(multi_class_labels=y, logits=output)
